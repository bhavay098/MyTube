...
// If a valid access token exists, attach the user to req.user.
// If there is no token or the token is invalid, continue as an anonymous request.
export const optionalJWT = async (req, _, next) => {
  try {
    ...
    // Ignore auth errors on public routes and treat the request as anonymous.
  }

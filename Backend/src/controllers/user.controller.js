...
} from "../utils/cloudinary.js"; // Helpers to upload & delete files from Cloudinary
...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    ); // Handle any errors while generating tokens
  }
};
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    // Use array.some() to check if any field is empty or contains only whitespace
  ...
  const createdUser = await User.findById(user._id).select(
    // select() with minus sign (-) excludes those fields from the result
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  return res
    ...
    .cookie("accessToken", accessToken, cookieOptions) // Set access token cookie
    .cookie("refreshToken", refreshToken, cookieOptions) // Set refresh token cookie
    .json(
      new ApiResponse(
        200,
        {
...
//   path: "/",
  ...
  return res
    .status(200) // HTTP 200 = Success
    .clearCookie("accessToken", cookieOptions) // Remove access token cookie from browser
    .clearCookie("refreshToken", cookieOptions) // Remove refresh token cookie from browser
    .json(
      new ApiResponse(
        200,
...
const refreshAccessToken = asyncHandler(async (req, res) => {
  const incomingRefreshToken =
    req.cookies.refreshToken || req.body.refreshToken; // 1. Try to read refreshToken from cookies (browser) or request body (mobile apps / APIs)
  ...
  ); // Decodes the payload data (user ID, etc.)
  ...
  if (!user) {
    // 5. If user doesn't exist in DB, reject the request as user might have been deleted after refresh token was issued
...
//   path: "/",
  ...
  return res
    .status(200) // HTTP 200 = Success
    .cookie("accessToken", accessToken, cookieOptions) // Update access token cookie
    .cookie("refreshToken", refreshToken, cookieOptions) // Update refresh token cookie
    .json(
      new ApiResponse(200, {}, "Access token refreshed"), // Success message
    );
});
...
const changeCurrentPassword = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    // Check if user exists in database (maybe account was deleted)
  ...
  if (!isPasswordCorrect) {
    // If the old password is wrong, throw an error
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!coverImage.url) {
    // Check if the upload returned a valid URL
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  if (!username?.trim()) {
    // If username is missing or just empty spaces, throw error
  ...
  const channel = await User.aggregate([
    // channel stores the result of the aggregation query which returns an array
    {
    ...
    },
    {
      ...
      $addFields: {
        ...
        isSubscribed: {
          ...
          $cond: {
            // If current user's ID exists in the subscribers list, return true
    ...
    },
    {
      ...
      $project: {
        // Note: 1 means include, 0 means exclude
...
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    // Use MongoDB aggregation pipeline to fetch watch history with nested data
    {
    ...
    },
    {
      ...
      $lookup: {
        ...
        pipeline: [
          // Sub-pipeline for each matched video
          {
            ...
            $lookup: {
              ...
              pipeline: [
                // Sub-pipeline: Control what owner data to include
                {
